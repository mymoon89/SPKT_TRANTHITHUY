...
function validate() {
	...
	if (sType == "CodeMCSS") {
		...
		if (String(counter) == String(correctAns)) {
			...
			incorrectCount++;
			//var temp = Math.floor(Number(_level0.emptyHolder_mc.slide_mc.score_mc.wrongCount.text));
			var temp = _level0.emptyHolder_mc.slide_mc.score_mc.wrongCount.text;
			trace(temp+" is the wrong count");
			_level0.emptyHolder_mc.slide_mc.score_mc.wrongCount.text = Number(temp)+Number(incorrectCount);
			//_level0.emptyHolder_mc.slide_mc.score_mc.wrongCount.text = 2;
			//trace(_level0.emptyHolder_mc.slide_mc.score_mc.wrongCount.text+" is the updated wrong count count");
			wrongScore = _level0.emptyHolder_mc.slide_mc.score_mc.wrongCount.text;
			rightScore = _level0.emptyHolder_mc.slide_mc.score_mc.rightCount.text;
			bookmark.data.rightCount = rightScore;
			bookmark.data.wrongCount = wrongScore;
		...
		if (scount == sresult.length && userArray.length == sresult.length) {
			...
			correctCount++;
			// this is where the score gets updated
			var temp = Math.floor(Number(_level0.emptyHolder_mc.slide_mc.score_mc.rightCount));
			trace("you have got it right: but with some value"+temp);
			_level0.emptyHolder_mc.slide_mc.score_mc.rightCount = Number(temp)+Number(correctCount);
			rightScore = _level0.emptyHolder_mc.slide_mc.score_mc.rightCount;
			wrongScore = _level0.emptyHolder_mc.slide_mc.score_mc.wrongCount;
			bookmark.data.rightCount = rightScore;
			bookmark.data.wrongCount = wrongScore;
		} else {
			...
			tick_mc.feedback_txt.text = incorrectFeedback;
			incorrectCount++;
			var temp = Math.floor(Number(_level0.emptyHolder_mc.slide_mc.score_mc.wrongCount));
			trace("you have got it wrong: but with some value"+temp);
			_level0.emptyHolder_mc.slide_mc.score_mc.wrongCount = Number(temp)+Number(incorrectCount);
			wrongScore = _level0.emptyHolder_mc.slide_mc.score_mc.wrongCount;
			rightScore = _level0.emptyHolder_mc.slide_mc.score_mc.rightCount;
			bookmark.data.rightCount = rightScore;
			bookmark.data.wrongCount = wrongScore;
